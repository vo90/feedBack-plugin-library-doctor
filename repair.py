...
class RepairService:
    ...
    def _record_recovered_transaction(
        ...
        *,
        ...
        backup_id = transaction["backup_id"]
        if any(item.get("backup_id") == backup_id for item in history):
            return True
        summary = metadata.get("summary")
        if not isinstance(summary, dict):
            summary = {}
        ...
        item = {
            ...
            "action": "repair" if repair_committed else "restore",
            "outcome": "success" if repair_committed else "restored",
            "completed_at": time.time(),
            "package": transaction["package"],
    ...
    def _commit(
        ...
        *,
        ...
        if package_path.is_file():
            ...
            self._emit_transaction_barrier(
                "package_committed", package=package_name, operation="archive"
            )
            return
        ...
        try:
            ...
            for member_path, raw in replacements.items():
                ...
                self._emit_transaction_barrier(
                    "member_replaced",
                    package=package_name,
                    operation=(transaction or {}).get("operation", "repair"),
                    member_path=member_path,
