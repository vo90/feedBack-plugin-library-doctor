...
def decrypt_sng(data, key):
    ...
    if length > 32 * 1024 ** 2:
        raise ValueError("Source SNG exceeds the 32 MiB expansion bound")
    inflater = zlib.decompressobj()
    payload = inflater.decompress(payload, length + 1)
    if len(payload) != length or not inflater.eof:
        raise ValueError("Source SNG expansion does not match its declared length")
    return payload
