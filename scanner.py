...
REPAIR_WORKSPACE_PREFIXES = (
    ".library-doctor-work-",
    ".library-doctor-repair-",  # Pre-public builds used this discoverable layout.
)
...
_SCAN_POLICY_MODULE_NAME = "_library_doctor_scan_policy"
_scan_policy = sys.modules.get(_SCAN_POLICY_MODULE_NAME)
if _scan_policy is None:
    _scan_policy_spec = importlib.util.spec_from_file_location(
        _SCAN_POLICY_MODULE_NAME,
        Path(__file__).resolve().with_name("library_doctor_scan_policy.py"),
    )
    _scan_policy = importlib.util.module_from_spec(_scan_policy_spec)
    sys.modules[_SCAN_POLICY_MODULE_NAME] = _scan_policy
    _scan_policy_spec.loader.exec_module(_scan_policy)
choose_worker_policy = _scan_policy.choose_worker_policy

_REPORT_CACHE_MODULE_NAME = "_library_doctor_report_cache"
_report_cache = sys.modules.get(_REPORT_CACHE_MODULE_NAME)
if _report_cache is None:
    _report_cache_spec = importlib.util.spec_from_file_location(
        _REPORT_CACHE_MODULE_NAME,
        Path(__file__).resolve().with_name("library_doctor_report_cache.py"),
    )
    _report_cache = importlib.util.module_from_spec(_report_cache_spec)
    sys.modules[_REPORT_CACHE_MODULE_NAME] = _report_cache
    _report_cache_spec.loader.exec_module(_report_cache)
_ReportCache = _report_cache.ReportCache
        ...
        *,
        ...
        for dirpath, dirnames, filenames in os.walk(
            ...
            dirnames[:] = [
                ...
                if not name.casefold().startswith(REPAIR_WORKSPACE_PREFIXES)
            ]
